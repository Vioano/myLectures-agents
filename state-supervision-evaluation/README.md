...
```text
state-supervision-evaluation/runs/<run-id>/
  run-manifest.json
  observations.jsonl
  retrospective.md
  evidence-index.json
...
```bash
python3 state-supervision-evaluation/scripts/init_episode_run.py \
  --episode-id 0013 \
  --slug <episode-slug> \
...
```bash
python3 state-supervision-evaluation/scripts/check_run_pack.py \
  state-supervision-evaluation/runs/<run-id> \
  --ready
```
