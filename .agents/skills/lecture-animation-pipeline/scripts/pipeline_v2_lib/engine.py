#!/usr/bin/env python3
"""Evidence-bound helpers for the canonical lecture-animation-pipeline Skill."""
...
from .design_readiness import (
    design_readiness_draft_data,
    scene_complexity_gate_data,
    validate_design_readiness_data,
)
from .episode_ops import (
...
STRUCTURAL_REPRESENTATION_AXES = {
    "primary_math_objects",
    "stage_topology",
    "display_mapping_modes",
    "causal_chain",
}
SUPERVISOR_ASSIGNMENT_STATES = {
    "active",
    "idle",
    "completed",
    "blocked",
...
SUPERVISOR_TASK_STATES = {"pending", "active", "completed", "blocked", "cancelled"}
SUPERVISOR_REPLACEMENT_STATUSES = {
    "authorized",
    "consumed",
    "cancelled",
    "expired",
...
SCREEN_TEXT_NARRATION_EXEMPT_ROLES = {
    "math_formula",
    "object_label",
    "axis_or_tick",
    "parameter_value",
}
SCREEN_TEXT_PRODUCER_MARKERS = (
    "这个动画",
    "本动画",
    "动画想表达",
    "为了帮助观众",
...
def narrated_action_mentions(narration: str) -> list[dict[str, Any]]:
    """Return non-overlapping strong action terms in spoken order."""
    occupied: set[int] = set()
    raw: list[tuple[int, int, str, str]] = []
    for token, action_kind in NARRATED_ACTION_TERMS:
        start = 0
        while True:
            index = narration.find(token, start)
            if index < 0:
                break
            span = set(range(index, index + len(token)))
            if not span & occupied:
                raw.append((index, index + len(token), token, action_kind))
                occupied.update(span)
            start = index + len(token)
    raw.sort(key=lambda item: (item[0], -(item[1] - item[0])))
    counts: Counter[tuple[str, str]] = Counter()
    mentions: list[dict[str, Any]] = []
    for start, end, token, action_kind in raw:
        key = (token, action_kind)
        counts[key] += 1
        mentions.append(
            {
                "spoken_token": token,
...
def build_history_records(repo_root: Path) -> list[dict[str, Any]]:
    ...
    old_references = (
        ...
        / ".agents"
        / "skills"
        / "lecture-animation-pipeline-legacy"
        / "references"
    )
...
def issue_tag_hits(issue_text: str, tags: set[str]) -> list[str]:
    ...
    hits: list[str] = []
    # Profile hashes must be reproducible across Python processes. Iterating a
    # set directly leaks randomized hash order into `matched_tags`, which can
    # invalidate an otherwise unchanged design chain on recompilation.
...
def issue_status_for_scene(issue: dict[str, Any], profile: dict[str, Any]) -> tuple[str, str]:
    """Resolve a global umbrella issue without stalling repaired sibling scenes.

    The umbrella remains open at episode level.  When it carries explicit
    `scene_statuses`, the current scene's status is the hard-gate authority.
    Missing scene entries deliberately fall back to the umbrella status.
    """

    umbrella_status = str(issue.get("status", "unknown"))
    if not issue.get("global_scope"):
        return umbrella_status, "issue_status"
    scene_statuses = issue.get("scene_statuses")
    if not isinstance(scene_statuses, dict):
        return umbrella_status, "umbrella_status"
    scene_slug = str(profile.get("context", {}).get("scene_slug", "")).strip()
    if not scene_slug or scene_slug not in scene_statuses:
        return umbrella_status, "umbrella_status_fallback"
    return str(scene_statuses[scene_slug]), "scene_status"
...
def compile_live_policy_data(episode_dir: Path, profile: dict[str, Any]) -> dict[str, Any]:
    ...
    if issues_dir.exists():
        for path in sorted(issues_dir.glob("*.json")):
            ...
            if match_scope == "implicit_risk_tag":
                ...
                continue
            effective_status, status_source = issue_status_for_scene(issue, profile)
            entries.append(
                {
                    "issue_id": issue.get("id", path.stem),
                    "pattern_key": issue.get("pattern_key", issue.get("standard_key", path.stem)),
                    ...
                    "umbrella_status": issue.get("status", "unknown")
                    if issue.get("global_scope")
                    else None,
    ...
    # `implicit_advisory_matches_omitted` is diagnostic retrieval telemetry.
    # By definition those loose matches do not enter the scene contract, so a
    # changing omitted count must not invalidate every frozen parallel scene.
    policy["policy_hash"] = live_policy_semantic_hash(policy)
    return policy


def live_policy_semantic_hash(policy: dict[str, Any]) -> str:
    payload = dict(policy)
    payload.pop("policy_hash", None)
    payload.pop("implicit_advisory_matches_omitted", None)
...
def validate_live_policy_hash(policy: dict[str, Any]) -> bool:
    ...
    legacy_hash = object_hash(payload)
    return (
        policy.get("schema") == "lecture-animation-live-policy-v2"
        and bool(expected)
        and expected in {legacy_hash, live_policy_semantic_hash(policy)}
    )
...
def attach_autopilot_contract(
    ...
    try:
        duration = float(profile.get("context", {}).get("duration"))
    except (TypeError, ValueError):
        duration = 0.0
    if duration <= 0:
        raise PipelineError(
            "autopilot profile requires a positive scene duration from timeline.scene_groups "
            "or review/v2/<scene_slug>/timeline_fragment.json"
        )
    result = dict(profile)
...
def scene_timing_context(
    episode_dir: Path,
    scene_slug: str,
    group: dict[str, Any],
) -> dict[str, Any]:
    """Resolve scene timing from the coarse group or its precise local fragment."""
    start = group.get("start")
    end = group.get("end")
    duration = group.get("duration")
    timing_source: str | None = None
    planning_status = str(group.get("planning_status", "")).strip().lower()
    group_timing_is_provisional = planning_status in {
        "provisional",
        "coarse",
        "outline_draft",
        "draft",
    ...
    try:
        ...
        if duration in (None, "") and numeric_end > numeric_start:
            duration = numeric_end - numeric_start
    except (TypeError, ValueError):
        pass
    if duration not in (None, "") and not group_timing_is_provisional:
        try:
            if float(duration) > 0:
                timing_source = "timeline.scene_groups"
        except (TypeError, ValueError):
            duration = None

    scene_review_dir = episode_dir / "review" / "v2" / scene_slug
    fragment_candidates = [
        scene_review_dir / "timeline_fragment.json",
        scene_review_dir / "narration" / "timeline_fragment.json",
    ]
    fragment_path = next((path for path in fragment_candidates if path.is_file()), fragment_candidates[0])
    fragment: dict[str, Any] | None = None
    if timing_source is None and fragment_path.is_file():
        fragment = load_json(fragment_path)
        # Evaluate animation contracts against the rendered scene clock. Audio
        # duration is evidence, but must not truncate a longer visual tail.
        for field in (
            ...
            try:
                candidate = float(fragment.get(field))
            except (TypeError, ValueError):
                continue
            if candidate > 0:
                duration = candidate
                ...
                timing_source = relative_or_absolute(fragment_path, episode_dir.parent.parent)
                break
    # A provisional scene-group duration remains useful before exact audio
    # exists, but it must never outrank a precise local fragment. Retain the
    # old planning fallback only when no usable fragment is available.
    if timing_source is None and group_timing_is_provisional and duration not in (None, ""):
        try:
            if float(duration) > 0:
                timing_source = "timeline.scene_groups:provisional_fallback"
        except (TypeError, ValueError):
            duration = None
    return {
        "start": start,
        ...
        "timing_fragment_hash": object_hash(fragment) if fragment is not None else None,
    }


def scene_narration_context(
    repo_root: Path,
    episode_dir: Path,
    scene_slug: str,
    coarse_narration: str,
) -> dict[str, Any]:
    """Prefer exact scene-local speech over provisional coarse timeline prose."""
    scene_review_dir = episode_dir / "review" / "v2" / scene_slug
    fragment_candidates = [
        scene_review_dir / "timeline_fragment.json",
        scene_review_dir / "narration" / "timeline_fragment.json",
    ]
    for fragment_path in fragment_candidates:
        if not fragment_path.is_file():
            continue
        fragment = load_json(fragment_path)
        artifacts = fragment.get("artifacts", {})
        approved_narration = (
            artifacts.get("approved_narration", {})
            if isinstance(artifacts, dict)
            else {}
        )
        if isinstance(approved_narration, str):
            approved_narration = {"path": approved_narration}
        if isinstance(approved_narration, dict):
            raw_path = str(approved_narration.get("path", "")).strip()
            if raw_path:
                script_path = resolve_stored_path(raw_path, repo_root)
                if script_path.is_file():
                    narration = script_path.read_text(encoding="utf-8").strip()
                    if narration:
                        return {
                            "narration": narration,
                            "narration_source": relative_or_absolute(script_path, repo_root),
                            "narration_hash": object_hash(narration),
                        }

        approved_spoken_text = str(fragment.get("approved_spoken_text", "")).strip()
        if approved_spoken_text:
            return {
                "narration": approved_spoken_text,
                "narration_source": (
                    f"{relative_or_absolute(fragment_path, repo_root)}#approved_spoken_text"
                ),
                "narration_hash": object_hash(approved_spoken_text),
            }

        word_anchors = fragment.get("word_anchors", [])
        if isinstance(word_anchors, list):
            anchor_text = "\n\n".join(
                str(item.get("text", "")).strip()
                for item in word_anchors
                if isinstance(item, dict) and str(item.get("text", "")).strip()
            )
            if anchor_text:
                return {
                    "narration": anchor_text,
                    "narration_source": (
                        f"{relative_or_absolute(fragment_path, repo_root)}#word_anchors"
                    ),
                    "narration_hash": object_hash(anchor_text),
                }

    narration = coarse_narration.strip()
    return {
        "narration": narration,
        "narration_source": "timeline.segments",
        "narration_hash": object_hash(narration),
    }
...
def compile_profile_data(
    ...
    resolved_narration = scene_narration_context(
        repo_root,
        episode_dir,
        str(group.get("scene_slug", scene_slug)),
        coarse_narration,
    )
    narration = resolved_narration["narration"]
    resolved_timing = scene_timing_context(
        episode_dir,
        str(group.get("scene_slug", scene_slug)),
        group,
    )
    ...
    profile: dict[str, Any] = {
        ...
        "context": {
            ...
            "start": resolved_timing["start"],
            "end": resolved_timing["end"],
            "duration": resolved_timing["duration"],
            "timing_source": resolved_timing["timing_source"],
            "timing_fragment_hash": resolved_timing["timing_fragment_hash"],
            "role": group.get("role", ""),
            ...
            "narration": narration,
            "narration_source": resolved_narration["narration_source"],
            "narration_hash": resolved_narration["narration_hash"],
        },
        "tags": tags,
...
def validate_scene_plan_data(profile: dict[str, Any], plan: dict[str, Any]) -> list[str]:
    ...
    if int(profile.get("autopilot_contract_version") or 0) >= 3:
        errors.extend(
            validate_math_object_display_contract(
                plan, int(profile.get("autopilot_contract_version") or 0)
            )
        )
    if int(profile.get("autopilot_contract_version") or 0) >= 6:
        errors.extend(validate_representation_budget(plan))
        if "local_linearization" in tags and not any(
            isinstance(item, dict)
            and item.get("mode") == "local_zoom"
            and isinstance(item.get("zoom_contract"), dict)
            and item["zoom_contract"].get("refinement_goal") == "local_linearization"
            for item in plan.get("display_mappings", [])
        ):
            errors.append(
                "local_linearization scenes require a local_zoom display mapping with refinement_goal=local_linearization"
            )
    ...
    if int(profile.get("autopilot_contract_version") or 0) >= 7:
        text_contract = plan.get("screen_text_contract", {})
        semantic_items = text_contract.get("semantic_items", []) if isinstance(text_contract, dict) else []
        if (
            not isinstance(text_contract, dict)
            or text_contract.get("mode") != "exact"
            or len(str(text_contract.get("baseline_path", "")).strip()) < 8
            or len(str(text_contract.get("purpose", "")).strip()) < 20
            or text_contract.get("dynamic_payload_policy") != "runtime_registered"
            or not isinstance(semantic_items, list)
        ):
            errors.append(
                "autopilot v7 requires an exact screen_text_contract with semantic_items and runtime_registered dynamic text"
            )
            semantic_items = []
        semantic_keys: set[tuple[str, str]] = set()
        for item in semantic_items:
            if not isinstance(item, dict):
                errors.append("every screen_text_contract semantic item must be structured")
                continue
            constructor = str(item.get("constructor", ""))
            ...
            try:
                ...
                count = 0
            if not constructor or not payload or count < 1 or key in semantic_keys:
                errors.append("screen text semantic items require unique constructor/payload pairs and positive counts")
            if role not in SCREEN_TEXT_ROLES:
                errors.append(f"screen text payload {payload!r} uses unsupported semantic role {role!r}")
            if len(str(item.get("unique_visual_job", "")).strip()) < 12:
                errors.append(f"screen text payload {payload!r} requires one concrete unique_visual_job")
            if item.get("duplicates_narration") is not False or item.get("externalizes_production_intent") is not False:
                errors.append(f"screen text payload {payload!r} cannot duplicate narration or externalize production intent")
            semantic_keys.add(key)
        for key in ("narration_duplicate_payloads", "producer_intent_payloads"):
            if text_contract.get(key) != []:
                errors.append(f"screen_text_contract.{key} must be an explicit empty list")
    ...
    if int(profile.get("autopilot_contract_version") or 0) >= 7:
        narration = str(profile.get("context", {}).get("narration", ""))
        expected_mentions = narrated_action_mentions(narration)
        action_contracts = plan.get("narrated_action_contracts", [])
        if not isinstance(action_contracts, list):
            errors.append("narrated_action_contracts must be a list")
            action_contracts = []
        expected_keys = {
            (
                str(item.get("spoken_token")),
        ...
        for item in action_contracts:
            if not isinstance(item, dict):
                errors.append("every narrated_action_contract must be structured")
                continue
            token = str(item.get("spoken_token", ""))
            action_kind = str(item.get("action_kind", ""))
            try:
                occurrence_index = int(item.get("occurrence_index", 0))
            except (TypeError, ValueError):
                occurrence_index = 0
            key = (token, action_kind, occurrence_index)
            if key in actual_keys:
                errors.append(f"duplicate narrated action contract for {key!r}")
            actual_keys.add(key)
            anchor_id = str(item.get("word_anchor_id", ""))
            anchor = word_anchor_map.get(anchor_id)
            if not anchor_id or anchor is None:
                errors.append(f"narrated action {key!r} requires an existing word_anchor_id")
            else:
                anchor_token = str(anchor.get("token", ""))
                if token not in anchor_token and anchor_token not in token:
                    errors.append(f"narrated action {key!r} does not match word anchor token {anchor_token!r}")
                if anchor.get("evidence_type") != "runtime_action":
                    errors.append(f"narrated action {key!r} must use runtime_action evidence, not emphasis or formula replacement")
                if str(anchor.get("target_id", "")) != str(item.get("object_id", "")):
                    errors.append(f"narrated action {key!r} and its word anchor target different objects")
                if str(anchor.get("evidence_id", "")) != str(item.get("evidence_id", "")):
                    errors.append(f"narrated action {key!r} and its word anchor use different evidence ids")
            if item.get("delivery_mode") not in ACTION_DELIVERY_MODES:
                errors.append(f"narrated action {key!r} requires enacted, counterexample, or inhibition visual delivery")
            if len(str(item.get("object_id", "")).strip()) < 1:
                errors.append(f"narrated action {key!r} requires a mathematical object_id")
            if len(str(item.get("expected_visible_change", "")).strip()) < 12:
                errors.append(f"narrated action {key!r} requires a concrete expected_visible_change")
            if len(str(item.get("evidence_id", "")).strip()) < 1:
                errors.append(f"narrated action {key!r} requires a runtime evidence_id")
            if item.get("screen_text_policy") != "motion_not_duplicate_text":
                errors.append(f"narrated action {key!r} must forbid duplicate screen text as evidence")
            if not isinstance(item.get("measurement_contract"), dict):
                errors.append(f"narrated action {key!r} requires a measurement_contract")
        if actual_keys != expected_keys:
            missing = sorted(expected_keys - actual_keys)
            unexpected = sorted(actual_keys - expected_keys)
            if missing:
                errors.append("narrated_action_contracts missing detected actions: " + ", ".join(map(str, missing)))
            if unexpected:
                errors.append("narrated_action_contracts contain undetected actions: " + ", ".join(map(str, unexpected)))
        if expected_mentions and plan.get("timing_contract_version") != "word_anchor_v1":
            errors.append("scenes with narrated transformations must use exact word_anchor_v1 timing")
...
def validate_authoring_qc_data(
    ...
    if int(profile.get("autopilot_contract_version") or 0) >= 3:
        ...
        for mapping_id, mapping in planned_mappings.items():
            ...
            if int(profile.get("autopilot_contract_version") or 0) >= 6 and mapping.get("mode") == "local_zoom":
                zoom_plan = mapping.get("zoom_contract", {})
                zoom_check = check.get("zoom_contract_check", {})
                if not isinstance(zoom_check, dict):
                    add_issue(
                        "LOCAL_ZOOM_CONTRACT_MISSING",
                        f"display mapping {mapping_id!r} lacks runtime zoom-contract evidence",
                    )
                    continue
                try:
                    source_fraction = float(zoom_check.get("source_span_fraction"))
                    max_source_fraction = float(zoom_plan.get("max_source_span_fraction"))
                except (TypeError, ValueError):
                    source_fraction, max_source_fraction = 1.0, 0.0
                source_window = zoom_plan.get("source_window_math", {})
                try:
                    planned_fraction = float(source_window.get("source_span")) / float(
                        source_window.get("context_span")
                    )
                except (TypeError, ValueError, ZeroDivisionError, AttributeError):
                    planned_fraction = 1.0
                if (
                    source_fraction > max_source_fraction + 1e-9
                    or source_fraction <= 0.0
                    or abs(source_fraction - planned_fraction) > 1e-9
                ):
                    add_issue(
                        "LOCAL_INCREMENT_NOT_SMALL",
                        f"display mapping {mapping_id!r} source span {source_fraction:.4f} exceeds {max_source_fraction:.4f}",
                    )
                transform = zoom_plan.get("zoom_transform", {})
                scale = transform.get("scale", []) if isinstance(transform, dict) else []
                try:
                    screen_magnification = min(abs(float(scale[0])), abs(float(scale[1])))
                except (TypeError, ValueError, IndexError):
                    screen_magnification = 0.0
                if screen_magnification <= 1.0:
                    add_issue(
                        "LOCAL_ZOOM_MAGNIFICATION_MISSING",
                        f"display mapping {mapping_id!r} does not magnify the true local source interval",
                    )
                if set(map(str, zoom_check.get("driver_ids", []))) != set(
                    map(str, source_window.get("driver_ids", []))
                ):
                    add_issue(
                        "LOCAL_ZOOM_DRIVER_MISMATCH",
                        f"display mapping {mapping_id!r} runtime drivers differ from the mathematical source",
                    )
                if str(zoom_check.get("math_state_hash", "")) != str(
                    source_window.get("math_state_hash", "")
                ):
                    add_issue(
                        "LOCAL_ZOOM_MATH_STATE_DRIFT",
                        f"display mapping {mapping_id!r} context and zoom do not share one mathematical state",
                    )
                planned_samples = {
                    str(sample.get("sample_id")): sample
                    for sample in zoom_plan.get("correspondence_samples", [])
                    if isinstance(sample, dict) and sample.get("sample_id")
                }
                runtime_samples = {
                    str(sample.get("sample_id")): sample
                    for sample in zoom_check.get("correspondence_samples", [])
                    if isinstance(sample, dict) and sample.get("sample_id")
                }
                if set(runtime_samples) != set(planned_samples):
                    add_issue(
                        "LOCAL_ZOOM_CORRESPONDENCE_MISSING",
                        f"display mapping {mapping_id!r} runtime correspondence ids differ from the plan",
                    )
                elif transform.get("kind") == "affine":
                    try:
                        affine_scale = [float(value) for value in transform.get("scale", [])]
                        affine_translation = [
                            float(value) for value in transform.get("translation", [])
                        ]
                        tolerance = float(zoom_check.get("coordinate_tolerance", 1e-6))
                    except (TypeError, ValueError):
                        affine_scale, affine_translation, tolerance = [], [], 1e-6
                    for sample_id, planned_sample in planned_samples.items():
                        runtime_sample = runtime_samples.get(sample_id, {})
                        try:
                            planned_source = [
                                float(value)
                                for value in planned_sample.get("source_coordinate", [])
                            ]
                            runtime_source = [
                                float(value)
                                for value in runtime_sample.get("source_coordinate", [])
                            ]
                            observed_zoom = [
                                float(value)
                                for value in runtime_sample.get(
                                    "observed_zoom_coordinate", []
                                )
                            ]
                            expected_zoom = [
                                planned_source[index] * affine_scale[index]
                                + affine_translation[index]
                                for index in range(2)
                            ]
                            source_error = max(
                                abs(planned_source[index] - runtime_source[index])
                                for index in range(2)
                            )
                            zoom_error = max(
                                abs(expected_zoom[index] - observed_zoom[index])
                                for index in range(2)
                            )
                        except (TypeError, ValueError, IndexError):
                            source_error = zoom_error = float("inf")
                        if source_error > tolerance or zoom_error > tolerance:
                            code = (
                                "LOCAL_ZOOM_CENTER_DRIFT"
                                if planned_sample.get("role") == "center"
                                else "LOCAL_ZOOM_BOUNDARY_MISMATCH"
                            )
                            add_issue(
                                code,
                                f"display mapping {mapping_id!r} correspondence {sample_id!r} does not match the declared transform",
                            )
                    if transform.get("orientation_policy") == "preserve" and any(
                        value <= 0 for value in affine_scale
                    ):
                        add_issue(
                            "LOCAL_ZOOM_ORIENTATION_DRIFT",
                            f"display mapping {mapping_id!r} reverses orientation despite preserve policy",
                        )
                if (
                    zoom_check.get("context_view_visible") is not True
                    or zoom_check.get("zoom_view_visible") is not True
                ):
                    add_issue(
                        "LOCAL_ZOOM_CONTEXT_MISSING",
                        f"display mapping {mapping_id!r} must show both global context and the focused view",
                    )
                if (
                    zoom_check.get("shared_anchor_verified") is not True
                    or zoom_check.get("same_source_identity_verified") is not True
                    or str(zoom_check.get("identity_anchor", ""))
                    != str(zoom_plan.get("identity_anchor", ""))
                ):
                    add_issue(
                        "LOCAL_ZOOM_IDENTITY_DRIFT",
                        f"display mapping {mapping_id!r} does not prove that the inset is the same local object",
                    )
                if zoom_plan.get("refinement_goal") in {"local_linearization", "limit_behavior"}:
                    samples = zoom_check.get("refinement_samples", [])
                    try:
                        required_samples = int(zoom_plan.get("min_refinement_samples"))
                    except (TypeError, ValueError):
                        required_samples = 3
                    if not isinstance(samples, list) or len(samples) < required_samples:
                        add_issue(
                            "LOCAL_LINEARIZATION_EVIDENCE_MISSING",
                            f"display mapping {mapping_id!r} requires at least {required_samples} refinement samples",
                        )
                    else:
                        try:
                            source_spans = [float(item.get("source_span")) for item in samples]
                            approximation_errors = []
                            for item in samples:
                                curve_value = float(item.get("curve_value"))
                                tangent_value = float(item.get("tangent_value"))
                                derived_error = abs(curve_value - tangent_value)
                                reported_error = float(item.get("approximation_error"))
                                if abs(derived_error - reported_error) > 1e-9:
                                    raise ValueError("reported approximation error is not derived")
                                approximation_errors.append(derived_error)
                        except (TypeError, ValueError, AttributeError):
                            source_spans, approximation_errors = [], []
                            add_issue(
                                "LOCAL_LINEARIZATION_EVIDENCE_MISSING",
                                f"display mapping {mapping_id!r} has malformed refinement samples",
                            )
                        if source_spans and (
                            any(
                                later >= earlier - 1e-12
                                for earlier, later in zip(source_spans, source_spans[1:])
                            )
                            or any(
                                later > earlier + 1e-12
                                for earlier, later in zip(
                                    approximation_errors, approximation_errors[1:]
                                )
                            )
                            or approximation_errors[-1] >= approximation_errors[0] - 1e-12
                        ):
                            add_issue(
                                "LOCAL_LINEARIZATION_NOT_CONVERGING",
                                f"display mapping {mapping_id!r} does not show decreasing source scale and approximation error",
                            )

        if int(profile.get("autopilot_contract_version") or 0) >= 6:
            budget = plan.get("representation_budget", {})
            runtime_representation_checks = {
                str(item.get("technique_id")): item
                for item in telemetry.get("representation_checks", [])
                if isinstance(item, dict) and item.get("technique_id")
            }
            for technique in budget.get("techniques", []) if isinstance(budget, dict) else []:
                if not isinstance(technique, dict):
                    continue
                technique_id = str(technique.get("technique_id", ""))
                check = runtime_representation_checks.get(technique_id)
                if check is None:
                    add_issue(
                        "REPRESENTATION_EVIDENCE_MISSING",
                        f"visual technique {technique_id!r} has no runtime evidence",
                    )
                    continue
                if check.get("passed") is not True:
                    add_issue(
                        "REPRESENTATION_VALUE_UNPROVEN",
                        f"visual technique {technique_id!r} failed its declared value test",
                    )
                if str(check.get("value_channel", "")) != str(
                    technique.get("value_channel", "")
                ):
                    add_issue(
                        "REPRESENTATION_VALUE_DRIFT",
                        f"visual technique {technique_id!r} runtime value channel differs from the plan",
                    )
                for plan_field, check_field, code in (
                    ("view_ids", "view_ids_observed", "REPRESENTATION_BUDGET_PLAN_MISMATCH"),
                    (
                        "math_object_ids",
                        "math_object_ids_observed",
                        "REPRESENTATION_BUDGET_PLAN_MISMATCH",
                    ),
                    (
                        "display_mapping_ids",
                    ...
                    ("driver_ids", "driver_ids_observed", "AUX_VIEW_DRIVER_DRIFT"),
                ):
                    if set(map(str, check.get(check_field, []))) != set(
                        map(str, technique.get(plan_field, []))
                    ):
                        add_issue(
                            code,
                            f"visual technique {technique_id!r} runtime {check_field} differs from the plan",
                        )
                planned_checkpoints = set(
                    map(str, technique.get("evidence_checkpoint_ids", []))
                )
                observed_checkpoints = set(
                    map(str, check.get("observed_checkpoint_ids", []))
                )
                if not planned_checkpoints <= observed_checkpoints:
                    add_issue(
                        "REPRESENTATION_CHECKPOINT_MISSING",
                        f"visual technique {technique_id!r} lacks planned QC checkpoints",
                    )
                if len(str(check.get("observed_value", "")).strip()) < 12:
                    add_issue(
                        "REPRESENTATION_VALUE_UNPROVEN",
                        f"visual technique {technique_id!r} lacks a concrete observed value",
                    )
                if len(str(check.get("removal_test_observation", "")).strip()) < 12:
                    add_issue(
                        "REPRESENTATION_REMOVAL_TEST_MISSING",
                        f"visual technique {technique_id!r} lacks a removal-test observation",
                    )
                if technique.get("value_channel") in {"cognitive", "continuity"}:
                    planned_carriers = set(map(str, technique.get("identity_carriers", [])))
                    verified_carriers = set(
                        map(str, check.get("identity_carriers_verified", []))
                    )
                    if not planned_carriers <= verified_carriers:
                        add_issue(
                            "REPRESENTATION_IDENTITY_UNVERIFIED",
                            f"visual technique {technique_id!r} did not verify every identity carrier",
                        )
                if technique.get("value_channel") == "aesthetic_finish" and (
                    check.get("became_primary_focal") is not False
                    or check.get("semantic_neutral") is not True
                    or check.get("protected_region_overlap") is not False
                ):
                    add_issue(
                        "AESTHETIC_FINISH_DISTRACTS",
                        f"aesthetic finish {technique_id!r} competes with or obscures mathematical evidence",
                    )
            finish_checks = {
                str(item.get("stage_state_id")): item
                for item in telemetry.get("visual_finish_checks", [])
                if isinstance(item, dict) and item.get("stage_state_id")
            }
            planned_state_ids = {
                str(item.get("id"))
                for item in plan.get("stage_states", [])
            ...
            for state_id in sorted(planned_state_ids):
                check = finish_checks.get(state_id)
                if check is None:
                    add_issue(
                        "VISUAL_FINISH_EVIDENCE_MISSING",
                        f"stage state {state_id!r} has no representative-frame visual-finish check",
                    )
                    continue
                if not isinstance(check.get("primary_object_ids"), list) or not check.get(
                    "primary_object_ids"
                ):
                    add_issue(
                        "VISUAL_FINISH_FOCAL_MISSING",
                        f"stage state {state_id!r} does not identify its primary visual object",
                    )
                for field, code in (
                    ("hierarchy_distinguishable", "VISUAL_FINISH_HIERARCHY_FLAT"),
                    ("thumbnail_readable", "VISUAL_FINISH_THUMBNAIL_UNREADABLE"),
                    ("negative_space_job_visible", "UNOWNED_NEGATIVE_SPACE"),
                    (
                        "animatic_composition_representative",
                        "ANIMATIC_FINISH_EXEMPTION_FORBIDDEN",
                    ),
                ):
                    if check.get(field) is not True:
                        add_issue(code, f"stage state {state_id!r} fails {field}")
                residue = check.get("generic_default_residue", [])
                if not isinstance(residue, list) or residue:
                    add_issue(
                        "GENERIC_DEFAULT_STYLE_RESIDUE",
                        f"stage state {state_id!r} retains unreviewed default styling",
                    )
                if len(str(check.get("observation", "")).strip()) < 20:
                    add_issue(
                        "VISUAL_FINISH_OBSERVATION_MISSING",
                        f"stage state {state_id!r} lacks a concrete full-size and thumbnail observation",
                    )
    ...
    for snapshot in snapshots:
        ...
        raw_atoms = snapshot.get("layout_atoms", [])
        atoms = [
            item
            for item in raw_atoms
            if isinstance(item, dict) and float(item.get("opacity", 1.0)) > 0.05
        ] if isinstance(raw_atoms, list) else []
        layout_atom_count += len(atoms)
        visible_atomic_parents = [
            item
            for item in objects
            if item.get("opacity", 1.0) > 0.05
            and str(item.get("kind")) in {"formula", "title", "body", "label", "tick_label"}
        ]
        if (
            {"formula_dense", "stage_dense", "repeat_rejected"} & tags
            and visible_atomic_parents
            and not atoms
        ):
            add_issue(
                "LAYOUT_ATOM_COVERAGE_MISSING",
                "formula/text-dense snapshot has no independently measured layout atoms",
                time=time,
                objects=[str(item.get("id")) for item in visible_atomic_parents],
            )
        for atom in atoms:
            atom_id = str(atom.get("atom_id", ""))
            parent_id = str(atom.get("parent_object_id", ""))
            bbox = atom.get("bbox")
            if not atom_id or not parent_id or parent_id not in object_by_id:
                add_issue(
                    "LAYOUT_ATOM_IDENTITY",
                    "layout atom requires a stable atom_id and a visible parent object",
                    time=time,
                    objects=[value for value in (atom_id, parent_id) if value],
                )
                continue
            if not isinstance(bbox, list) or len(bbox) != 4:
                add_issue(
                    "LAYOUT_ATOM_BOUNDS",
                    "layout atom bbox must contain four normalized values",
                    time=time,
                    objects=[atom_id],
                )
                continue
            try:
                atom["bbox"] = [float(value) for value in bbox]
            except (TypeError, ValueError):
                add_issue(
                    "LAYOUT_ATOM_BOUNDS",
                    "layout atom bbox must be numeric",
                    time=time,
                    objects=[atom_id],
                )
                continue
            if atom["bbox"][1] < subtitle_fraction:
                add_issue(
                    "LAYOUT_ATOM_SUBTITLE_INTRUSION",
                    "independent formula/text atom enters the reserved subtitle zone",
                    time=time,
                    objects=[atom_id],
                )
            if not bbox_inside(atom["bbox"], object_by_id[parent_id]["bbox"], padding=-0.004):
                add_issue(
                    "LAYOUT_ATOM_PARENT_DRIFT",
                    "independent formula/text atom leaves its declared parent geometry",
                    time=time,
                    objects=[atom_id, parent_id],
                )
        valid_atoms = [item for item in atoms if isinstance(item.get("bbox"), list) and len(item["bbox"]) == 4]
        for index, left in enumerate(valid_atoms):
            for right in valid_atoms[index + 1 :]:
                overlap = bbox_overlap_area(left["bbox"], right["bbox"])
                if overlap <= 0.00001:
                    continue
                left_id, right_id = str(left.get("atom_id")), str(right.get("atom_id"))
                if (
                    left.get("parent_object_id") == right.get("parent_object_id")
                    and left.get("kind") == right.get("kind") == "formula_fragment"
                ):
                    add_issue(
                        "FORMULA_ATOM_COLLISION",
                        f"independently positioned formula atoms overlap by {overlap:.5f}",
                        time=time,
                        objects=[left_id, right_id],
                    )
                elif left.get("kind") in {"solid", "formula_fragment"} and right.get("kind") in {
                    "solid",
                    "formula_fragment",
                }:
                    add_issue(
                        "LAYOUT_ATOM_COLLISION",
                        f"independent visual atoms overlap by {overlap:.5f}",
                        time=time,
                        objects=[left_id, right_id],
                    )
    ...
    if {"formula_dense", "stage_dense", "repeat_rejected"} & tags and layout_atom_count == 0:
        add_issue(
            "LAYOUT_ATOM_COVERAGE_MISSING",
            "dense scene telemetry contains no independently measured layout atoms at any runtime snapshot",
        )

    if "formula_dense" in tags and not any(
        isinstance(item, dict) for item in telemetry.get("formula_rows", [])
    ):
        add_issue(
            "FORMULA_ROW_AUDIT_COVERAGE_MISSING",
            "formula-dense scene telemetry contains no formula row-integrity audit",
        )
...
def validate_screen_text_semantics(
    ...
    for key in sorted(set(actual) & set(planned)):
        ...
        if any(marker.lower() in payload.lower() for marker in SCREEN_TEXT_PRODUCER_MARKERS):
            errors.append(f"screen text payload {payload!r} externalizes production intent")
...
def scene_registry_data(profile: dict[str, Any], plan: dict[str, Any], scene_production: dict[str, Any]) -> dict[str, Any]:
    result = {
        ...
        "narrated_action_contracts": plan.get("narrated_action_contracts", []),
        "screen_text_contract": plan.get("screen_text_contract", {}),
...
def review_capsule_data(
    ...
    challenge_candidates.sort(key=lambda item: (item[1], item[0]))
    selected_challenges = time_stratified_rows(challenge_candidates, 3, duration)
    challenges = [
        {
        ...
        for index, (layer, timestamp) in enumerate(selected_challenges)
    ]
    required_rules = [
        {
...
def load_review_session(path: Path, *, allow_stale_rules: bool = False) -> dict[str, Any]:
    ...
    if session.get("review_role") not in {
        "acceptance",
        "recusal_acceptance",
        "diagnostic_support",
    }:
    ...
    if (
        not allow_stale_rules
        and session.get("rules_registry_hash")
        and session.get("rules_registry_hash") != object_hash(load_rules())
    ):
...
def validate_repair_guidance(finding: dict[str, Any], manifest: dict[str, Any]) -> list[str]:
    ...
    required_changed = guidance.get("required_changed_artifacts", [])
    if not isinstance(required_changed, list):
        errors.append(f"{finding_id}: repair_guidance.required_changed_artifacts must be a list")
    else:
        unknown_required = sorted(set(map(str, required_changed)) - set(map(str, affected or [])))
        if unknown_required:
            errors.append(
                f"{finding_id}: required_changed_artifacts must be a subset of affected_artifacts: "
...
def author_repair_contract_data(
    repair_plan: dict[str, Any],
    author_self_review: dict[str, Any],
    author_attempt: dict[str, Any],
    manifest: dict[str, Any],
) -> dict[str, Any]:
    contract = repair_contract_data(repair_plan, manifest)
    contract.pop("review_hash", None)
    contract.pop("reviewer", None)
    contract.pop("reviewer_agent_id", None)
    contract["origin"] = "author_self_review"
    contract["repair_plan_hash"] = object_hash(repair_plan)
    contract["author_self_review_hash"] = object_hash(author_self_review)
    contract["author_self_review_attempt_id"] = author_attempt.get("attempt_id")
    contract["author_self_review_attempt_hash"] = object_hash(author_attempt)
    contract["author_agent_id"] = author_self_review.get("author_agent_id")
    source_hashes = {
        str(item.get("finding_id")): str(item.get("source_author_finding_hash"))
        for item in repair_plan.get("findings", [])
        if isinstance(item, dict)
    }
    for finding in contract.get("findings", []):
        if isinstance(finding, dict):
            finding["source_author_finding_hash"] = source_hashes.get(
                str(finding.get("finding_id"))
            )
    contract["contract_hash"] = object_hash(
        {key: value for key, value in contract.items() if key != "contract_hash"}
    )
    return contract


def validate_author_repair_plan_data(
    repair_plan: dict[str, Any],
    author_self_review: dict[str, Any],
    manifest: dict[str, Any],
    repo_root: Path,
    ...
    errors: list[str] = []
    if repair_plan.get("schema") != "lecture-animation-author-repair-plan-v1":
        errors.append("author repair plan schema is invalid")
    if repair_plan.get("verdict") != "revise":
        errors.append("author repair plan verdict must be revise")
    if repair_plan.get("manifest_hash") != manifest.get("manifest_hash"):
        errors.append("author repair plan is bound to another manifest")
    if repair_plan.get("author_self_review_hash") != object_hash(author_self_review):
        errors.append("author repair plan is bound to another rejected self-review")
    author_findings = {
        str(item.get("finding_id")): item
        for item in author_self_review.get("findings", [])
    ...
    if not author_findings or set(plan_findings) != set(author_findings):
        errors.append("author repair plan must expand every self-review finding exactly once")
    for finding_id, finding in plan_findings.items():
        source = author_findings.get(finding_id)
        if source is None:
            continue
        if finding.get("source_author_finding_hash") != object_hash(source):
            errors.append(
                f"{finding_id}: author repair plan source finding hash is stale"
            )
        if finding.get("source_author_finding") != source:
            errors.append(
                f"{finding_id}: author repair plan must preserve the exact source finding snapshot"
            )
        for field, source_value in source.items():
            if field not in finding:
                errors.append(
                    f"{finding_id}: author repair plan omitted immutable source field {field}"
                )
            elif finding.get(field) != source_value:
                errors.append(
                    f"{finding_id}: author repair plan changed immutable source field {field}"
                )
        errors.extend(validate_repair_guidance(finding, manifest))
    exhaustion = repair_plan.get("review_exhaustion")
    if not isinstance(exhaustion, dict):
        errors.append("author repair plan requires the sealed review exhaustion record")
    else:
        errors.extend(
            validate_review_exhaustion_data(
                exhaustion,
...
def validate_author_repair_contract_data(
    contract: dict[str, Any],
    repair_plan: dict[str, Any],
    author_self_review: dict[str, Any],
    author_attempt: dict[str, Any],
    manifest: dict[str, Any],
    repo_root: Path,
    recomputed_gate_errors: list[str] | None = None,
) -> list[str]:
    errors = validate_author_repair_plan_data(
        repair_plan, author_self_review, manifest, repo_root
    )
    if contract.get("schema") != "lecture-animation-repair-contract-v2":
        errors.append("author repair contract schema is invalid")
    if not validate_hashed_record(contract, "contract_hash"):
        errors.append("author repair contract hash is invalid")
    if contract.get("scene_slug") != manifest.get("scene_slug"):
        errors.append("author repair contract scene_slug does not match the baseline manifest")
    if contract.get("baseline_manifest_hash") != manifest.get("manifest_hash"):
        errors.append("author repair contract is bound to another baseline manifest")
    if contract.get("repair_plan_hash") != object_hash(repair_plan):
        errors.append("author repair contract repair_plan_hash is stale")
    exhaustion = repair_plan.get("review_exhaustion", {})
    if contract.get("review_exhaustion_hash") != exhaustion.get("exhaustion_hash"):
        errors.append("author repair contract review_exhaustion_hash is stale")
    expected_clusters = {
        str(item.get("root_issue_id")): object_hash(item)
        for item in exhaustion.get("clusters", [])
        if isinstance(item, dict)
    }
    supplied_clusters = {
        str(item.get("root_issue_id")): str(item.get("cluster_hash"))
        for item in contract.get("root_issue_clusters", [])
        if isinstance(item, dict)
    }
    if supplied_clusters != expected_clusters:
        errors.append("author repair contract must snapshot every root-issue cluster exactly once")
    if (
        author_self_review.get("schema") != "lecture-animation-author-self-review-v2"
        or author_self_review.get("verdict") != "revise_before_independent_review"
    ):
        errors.append("author repair contract requires a revise_before_independent_review self-review")
    if author_self_review.get("manifest_hash") != manifest.get("manifest_hash"):
        errors.append("author repair contract self-review is bound to another manifest")
    if author_attempt.get("schema") != "lecture-animation-author-self-review-attempt-v2":
        errors.append("author repair contract requires a canonical self-review attempt")
    if author_attempt.get("manifest_hash") != manifest.get("manifest_hash"):
        errors.append("author repair contract attempt is bound to another manifest")
    if author_attempt.get("draft_hash") != object_hash(author_self_review):
        errors.append("author repair contract attempt is bound to another self-review draft")
    expected_verification_payload = {
        "manifest_hash": manifest.get("manifest_hash"),
        "draft_hash": object_hash(author_self_review),
        "previous_review_hash": author_attempt.get("previous_review_hash"),
        "previous_author_self_review_hash": author_attempt.get(
            "previous_author_self_review_hash"
        ),
    }
    expected_verification_key = object_hash(expected_verification_payload)
    legacy_verification_key = object_hash(
        {
            "manifest_hash": manifest.get("manifest_hash"),
            "draft_hash": object_hash(author_self_review),
            "previous_review_hash": author_attempt.get("previous_review_hash"),
    ...
    verification_key = str(author_attempt.get("verification_key", ""))
    allowed_verification_keys = {expected_verification_key}
    if "previous_author_self_review_hash" not in author_attempt:
        allowed_verification_keys.add(legacy_verification_key)
    if verification_key not in allowed_verification_keys:
        errors.append("author repair contract attempt verification_key is invalid")
    expected_attempt_id = (
        f"author-self-review:{hashlib.sha1(verification_key.encode()).hexdigest()[:16]}"
    )
    if author_attempt.get("attempt_id") != expected_attempt_id:
        errors.append("author repair contract attempt id is not derived from its verification key")
    for field in ("owner", "author_agent_id", "author_model", "self_review_round"):
        if author_attempt.get(field) != author_self_review.get(field):
            errors.append(
                f"author repair contract attempt {field} does not match the rejected self-review"
            )
    if author_attempt.get("gate_accepted") is not False or author_attempt.get("verdict") != "self_review_revise":
        errors.append("author repair contract requires a gate-rejected self-review attempt")
    if int(author_attempt.get("findings_caught_before_handoff", 0) or 0) < 1:
        errors.append("author repair contract requires findings caught before handoff")
    if int(author_attempt.get("findings_caught_before_handoff", 0) or 0) != len(
        [
            item
            for item in author_self_review.get("findings", [])
            if isinstance(item, dict)
        ...
        errors.append(
            "author repair contract attempt finding count does not match the rejected self-review"
        )
    if (
        int(author_attempt.get("machine_gate_findings", 0) or 0) < 1
        or not isinstance(author_attempt.get("gate_errors"), list)
        or not author_attempt.get("gate_errors")
    ):
        errors.append("author repair contract requires recorded self-review gate errors")
    if recomputed_gate_errors is None:
        errors.append("author repair contract requires canonical self-review gate recomputation")
    elif author_attempt.get("gate_errors") != recomputed_gate_errors:
        errors.append("author repair contract attempt gate errors do not match canonical recomputation")
    elif int(author_attempt.get("machine_gate_findings", 0) or 0) != len(
        recomputed_gate_errors
    ):
        errors.append("author repair contract attempt gate error count is stale")
    if contract.get("origin") != "author_self_review":
        errors.append("author repair contract origin is invalid")
    if contract.get("author_self_review_hash") != object_hash(author_self_review):
        errors.append("author repair contract self-review hash is stale")
    if contract.get("author_self_review_attempt_id") != author_attempt.get("attempt_id"):
        errors.append("author repair contract attempt id is stale")
    if contract.get("author_self_review_attempt_hash") != object_hash(author_attempt):
        errors.append("author repair contract attempt hash is stale")
    expected_findings = {
        str(item.get("finding_id")): object_hash(item)
        for item in repair_plan.get("findings", [])
        if isinstance(item, dict)
    }
    supplied_findings = {
    ...
    if supplied_findings != expected_findings:
        errors.append("author repair contract must snapshot every plan finding exactly once")
    ...
    if supplied_source_hashes != expected_source_hashes:
        errors.append("author repair contract must bind every original author finding hash")
    return errors
...
def repair_execution_data(
    ...
    *,
    ...
    discovery_reviewer = str(contract.get("reviewer_agent_id") or "").strip()
    actor = str(repair_actor_agent_id or "").strip()
    verifier = str(planned_verifier_agent_id or "").strip()
    return {
        "mode": str(mode or "").strip(),
        "repair_actor_agent_id": actor,
        "editor_agent_ids": [actor] if actor else [],
        "discovery_reviewer_agent_id": discovery_reviewer or None,
        "planned_verifier_agent_id": verifier,
        "handoff_count": int(handoff_count),
        "recusal_required": actor == discovery_reviewer and bool(actor),
    }


def validate_repair_execution_data(
    execution: dict[str, Any], contract: dict[str, Any]
) -> list[str]:
    errors: list[str] = []
    mode = str(execution.get("mode", "")).strip()
    if mode not in {"same_author", "repair_surgeon", "reviewer_assisted"}:
        errors.append(
            "repair execution mode must be same_author, repair_surgeon, or reviewer_assisted"
        )
    actor = str(execution.get("repair_actor_agent_id", "")).strip()
    verifier = str(execution.get("planned_verifier_agent_id", "")).strip()
    editors = [
        str(value).strip()
        for value in execution.get("editor_agent_ids", [])
        if str(value).strip()
    ]
    discovery_reviewer = str(contract.get("reviewer_agent_id") or "").strip()
    if len(actor) < 4:
        errors.append("repair execution requires repair_actor_agent_id")
    if editors != ([actor] if actor else []):
        errors.append(
            "repair execution editor_agent_ids must exactly bind the repair actor"
        )
    if len(verifier) < 4:
        errors.append("repair execution requires planned_verifier_agent_id")
    if actor and verifier and actor == verifier:
        errors.append(
            "the repair actor is recused; planned_verifier_agent_id must differ"
        )
    if execution.get("discovery_reviewer_agent_id") != (
        discovery_reviewer or None
    ):
        errors.append("repair execution discovery reviewer binding is stale")
    if mode == "reviewer_assisted":
        if not discovery_reviewer or actor != discovery_reviewer:
            errors.append(
                "reviewer_assisted repair must be executed by the bound discovery reviewer"
            )
        if execution.get("recusal_required") is not True:
            errors.append("reviewer_assisted repair must require reviewer recusal")
    elif actor and discovery_reviewer and actor == discovery_reviewer:
        errors.append(
            "a discovery reviewer who edits production code must use reviewer_assisted mode"
        )
    if not isinstance(execution.get("handoff_count"), int) or int(
        execution.get("handoff_count", -1)
    ) < 0:
        errors.append("repair execution handoff_count must be a nonnegative integer")
    if mode in {"same_author", "repair_surgeon"} and int(
        execution.get("handoff_count", 0) or 0
    ) < 1:
        errors.append(
            "same_author and repair_surgeon modes must record at least one review-to-repair handoff"
        )
    return errors


def repair_response_draft_data(
    contract: dict[str, Any],
...
def validate_repair_response_data(
    ...
    if int(contract.get("repair_execution_contract_version", 0) or 0) >= 1:
        execution = response.get("repair_execution")
        if not isinstance(execution, dict):
            errors.append("repair response requires a sealed repair_execution record")
        else:
            errors.extend(validate_repair_execution_data(execution, contract))
    ...
    for finding_id in sorted(set(contract_map) & set(response_map)):
        ...
        required_changed = set(map(str, guidance.get("required_changed_artifacts", [])))
        ...
        if not required_changed <= changed:
            ...
            errors.append(f"{finding_id}: explicitly required artifacts were not updated: {', '.join(missing)}")
...
def repair_gate_data(
    ...
    errors: list[str] = []
    if (
        contract.get("schema") != "lecture-animation-repair-contract-v2"
        or not validate_hashed_record(contract, "contract_hash")
    ):
        errors.append("repair contract schema or hash is invalid")
    errors.extend(validate_repair_response_data(response, contract, current_manifest))
    gate: dict[str, Any] = {
...
def review_strategy_data(
    ...
    scene_attempts = [
        ...
        if str(row.get("scene_slug", "")) == scene_slug
...
def review_coverage_anchors(plan: dict[str, Any], duration: float) -> dict[str, list[float]]:
    ...
    return {
        ...
        "visual_finish": dedupe_times([opening, ending, *state_mids, *transition_mids]),
    }
...
def planned_object_ids(plan: dict[str, Any]) -> list[str]:
    return sorted(
        ...
        | {
            ...
            for item in plan.get("beats", [])
            if isinstance(item, dict)
            for object_id in item.get("pointing_target_ids", [])
            if str(object_id)
        }
    )


def self_review_claims(plan: dict[str, Any], layer: str) -> list[dict[str, Any]]:
    region_owners = {
        str(item.get("name")): str(item.get("primary_object"))
        for item in plan.get("stage_regions", [])
        if isinstance(item, dict) and item.get("name") and item.get("primary_object")
    }
    ...
    if layer == "layout":
        for item in plan.get("stage_states", []):
            if not isinstance(item, dict):
                continue
            state_id = str(item.get("id") or item.get("state_id") or "")
            objects = [
                region_owners.get(str(region.get("region")), "")
                for region in item.get("active_regions", [])
                if isinstance(region, dict)
            ]
            if state_id:
                claims.append(
                    {
                        "claim_id": f"stage-state:{state_id}",
                        "claim_type": "stage_state_clearance",
                        "object_ids": sorted(set(objects) - {""}),
                        "metric_id": "minimum_clearance_normalized",
                        "comparator": "at_least",
                        "anchor_times": [
                            (
    ...
    elif layer == "math_object":
        sources = plan.get("math_object_invariants", []) or plan.get("visual_bindings", [])
        for index, item in enumerate(sources, 1):
            if not isinstance(item, dict):
                continue
            claim_id = str(
                item.get("invariant_id")
                or item.get("display_mapping_id")
                or f"math-binding-{index}"
            )
            object_id = str(item.get("object_id") or item.get("visual_object_id") or "")
            claims.append(
                {
                    "claim_id": f"math:{claim_id}",
                    "claim_type": "math_invariant",
                    "object_ids": [object_id] if object_id else [],
                    "metric_id": "invariant_error",
                    "comparator": "at_most",
                    "anchor_times": [
                        float(value)
                        for value in item.get("checkpoints", [])
    ...
    elif layer == "timing_attention":
        for index, item in enumerate(plan.get("clause_locks", []), 1):
            if not isinstance(item, dict):
                continue
            cue_id = str(item.get("cue_id") or f"clause-{index}")
            object_id = str(item.get("object_id") or "")
            claims.append(
                {
                    "claim_id": f"clause-lock:{cue_id}",
                    "claim_type": "word_visual_anchor",
                    "object_ids": [object_id] if object_id else [],
                    "metric_id": "anchor_drift_seconds",
                    "comparator": "at_most",
                    "anchor_times": (
                        [float(item.get("spoken_start"))]
                        if isinstance(item.get("spoken_start"), (int, float))
                        else []
                    ),
                }
            )
    elif layer == "novice_causality":
        for index, item in enumerate(plan.get("beats", []), 1):
            if not isinstance(item, dict):
                continue
            beat_id = str(item.get("beat_id") or f"beat-{index}")
            claims.append(
                {
                    "claim_id": f"beat:{beat_id}",
                    "claim_type": "cause_result_settle",
                    "object_ids": [
                        str(value)
                        for value in item.get("pointing_target_ids", [])
                        if str(value)
                    ],
                    "metric_id": "cause_result_settle_seconds",
                    "comparator": "at_least",
                    "anchor_times": [
                        (
                            float(item.get("start", 0.0))
        ...
        for item in plan.get("stage_states", []):
            ...
            primary_objects = [
                region_owners.get(str(region.get("region")), "")
                for region in item.get("active_regions", [])
                if isinstance(region, dict) and region.get("salience") == "primary"
            ]
            if state_id:
                claims.append(
                    {
                        "claim_id": f"visual-finish:{state_id}",
                        "claim_type": "primary_visual_hierarchy",
                        "object_ids": sorted(set(primary_objects) - {""}),
                        "metric_id": "primary_visual_span_fraction",
                        "comparator": "at_least",
                        "anchor_times": [
                            (
                                float(item.get("start", 0.0))
    ...
    claims = [
        claim
        for claim in claims
        if isinstance(claim.get("object_ids"), list) and claim.get("object_ids")
    ]
    if not claims:
        objects = planned_object_ids(plan) or ["primary_math_object"]
        claims.append(
            {
                "claim_id": f"{layer}:primary-contract",
                "claim_type": f"{layer}_contract",
                "object_ids": objects[:2],
                "metric_id": f"{layer}_contract_residual",
                "comparator": "at_most",
                "anchor_times": [],
            }
        )
...
def evenly_spaced_sample_indices(length: int, count: int) -> list[int]:
    """Select deterministic samples across a sequence, including both ends when possible."""
    if length <= 0 or count <= 0:
        return []
    if count == 1:
        return [(length - 1) // 2]
    if length < count:
        raise PipelineError("cannot select more distinct samples than available candidates")
    return [round(index * (length - 1) / (count - 1)) for index in range(count)]


def time_stratified_rows(
    rows: list[tuple[str, float]], count: int, duration: float
) -> list[tuple[str, float]]:
    """Choose distinct rows nearest the centers of equal time strata."""
    if not rows or count <= 0:
        return []
    available = list(rows)
    selected: list[tuple[str, float]] = []
    effective_count = min(count, len(available))
    for index in range(effective_count):
        target = duration * (index + 0.5) / effective_count
        chosen = min(
            available,
...
def self_review_probe_draft_data(
    ...
    for layer, timestamps in review_coverage_anchors(plan, duration).items():
        claims = self_review_claims(plan, layer)
        claim_rows = sorted(
            [
                (round(float(timestamp), 3), claim)
                for claim in claims
                for timestamp in claim.get("anchor_times", [])
                if 0.0 <= float(timestamp) <= duration
            ],
            key=lambda row: (row[0], str(row[1].get("claim_id", ""))),
        )
        if not claim_rows:
            claim_rows = [
                (round(float(timestamp), 3), claims[0])
                for timestamp in timestamps
            ]
        while len(claim_rows) < minimum_per_layer:
            base_timestamp, base_claim = claim_rows[-1]
            offset = 0.1 * len(claim_rows)
            candidate = min(duration, base_timestamp + offset)
            if candidate == base_timestamp:
                candidate = max(0.0, base_timestamp - offset)
            claim_rows.append((round(candidate, 3), base_claim))
        selected_indices = evenly_spaced_sample_indices(
            len(claim_rows), minimum_per_layer
        )
        selected_rows = [claim_rows[index] for index in selected_indices]
        for index, (preferred_timestamp, claim) in enumerate(selected_rows, 1):
            available_candidates = (
                [preferred_timestamp]
                if preferred_timestamp not in used_timestamps
                else []
            )
            if not available_candidates:
                # Hard-gate layers can share an authored anchor. Challenge
                # nearby decoded frames instead of stealing another selected
                # anchor from this layer or reusing one frame globally.
                offsets = [
                    step * direction
                ...
                available_candidates = [
                    round(preferred_timestamp + offset, 3)
                    for offset in offsets
                    if 0.0 <= preferred_timestamp + offset <= duration
                    and round(preferred_timestamp + offset, 3) not in used_timestamps
                ]
            if not available_candidates:
                raise PipelineError(
                    "cannot allocate distinct self-review timestamps across hard-gate layers"
                )
            timestamp = min(
                available_candidates,
                key=lambda value: (abs(value - preferred_timestamp), value),
            )
...
def validate_self_review_probe_data(
    ...
    for item in probe.get("probes", []):
        ...
        if expected_probe is None:
            ...
            item.get(field) != expected_probe.get(field)
            for field in ("claim_id", "claim_type", "metric_id", "comparator", "object_ids")
        ):
            errors.append(
                f"{probe_id or layer}: semantic target was changed after the CLI selected it"
            )
        ...
        if isinstance(evidence, dict):
            evidence_paths.append(str(evidence.get("frame_path", "")).strip())
            evidence_timestamps.append(timestamp)
            errors.extend(
            ...
            if check_type != "numeric":
                ...
                try:
                    ...
                    if comparator == "at_most":
                        computed_pass = actual_value <= expected_value + tolerance_value
                    elif comparator == "at_least":
                        computed_pass = actual_value + tolerance_value >= expected_value
                    elif comparator == "within_absolute":
                        computed_pass = abs(actual_value - expected_value) <= tolerance_value
                    else:
                        errors.append(f"{probe_id or layer}: numeric comparator is invalid")
                        computed_pass = False
    ...
    if len(evidence_paths) != len(set(evidence_paths)) or any(not value for value in evidence_paths):
        errors.append("every self-review probe must use a distinct decoded frame path")
    if (
        len(evidence_timestamps) != len(set(evidence_timestamps))
        or any(value < 0 for value in evidence_timestamps)
    ):
        errors.append("every self-review probe must use a distinct CLI-selected timestamp")
...
def validate_author_self_review_data(
    ...
    previous_author_self_review: dict[str, Any] | None = None,
    ...
    if previous_review is not None and previous_author_self_review is not None:
        errors.append("repair self-review cannot bind both an independent review and an author self-review rejection")
    if previous_review is not None:
        ...
        if any(item is None for item in supplied_bundle):
            ...
            if repair_contract.get("origin") == "author_self_review":
                errors.append("independent-review repair cannot use an author-origin contract")
            if (
                repair_contract.get("schema") != "lecture-animation-repair-contract-v2"
                or not validate_hashed_record(repair_contract, "contract_hash")
            ):
                errors.append("repair contract schema or hash is invalid")
            ...
            if repair_context.get("repair_execution") != repair_response.get(
                "repair_execution"
            ):
                errors.append(
                    "repair self-review contains a stale repair_execution record"
                )
    elif previous_author_self_review is not None:
        if (
            previous_author_self_review.get("schema") != "lecture-animation-author-self-review-v2"
            or previous_author_self_review.get("verdict") != "revise_before_independent_review"
        ):
            errors.append("previous author self-review must be a revise_before_independent_review draft")
        expected_author_hash = object_hash(previous_author_self_review)
        if (
            not isinstance(repair_context, dict)
            or repair_context.get("previous_author_self_review_hash") != expected_author_hash
        ):
            errors.append("repair self-review is not bound to the previous author self-review rejection")
        if isinstance(repair_context, dict) and repair_context.get("previous_review_hash"):
            errors.append("author-origin repair self-review cannot claim an independent previous review")
        for field in ("repair_contract_hash", "repair_response_hash", "repair_gate_hash"):
            if not isinstance(repair_context, dict) or len(str(repair_context.get(field, "")).strip()) < 16:
                errors.append(f"author-origin repair self-review requires {field}")
        previous_findings = {
            str(item.get("finding_id"))
            for item in previous_author_self_review.get("findings", [])
            if isinstance(item, dict) and item.get("finding_id")
        }
        resolutions = {
            str(item.get("finding_id")): item
        ...
        if set(resolutions) != previous_findings:
            errors.append("author-origin repair self-review must resolve every self-review finding exactly once")
        for finding_id, resolution in resolutions.items():
            if len(str(resolution.get("change", "")).strip()) < 16:
                errors.append(f"author-origin repair resolution {finding_id!r} requires a concrete change")
            timestamps = resolution.get("evidence_timestamps", [])
            if not isinstance(timestamps, list) or not timestamps:
                errors.append(f"author-origin repair resolution {finding_id!r} requires timestamped evidence")
        supplied_bundle = (repair_contract, repair_response, repair_gate)
        if any(item is None for item in supplied_bundle):
            errors.append("author-origin repair contract, response, and gate must be supplied together")
        else:
            assert repair_contract is not None and repair_response is not None and repair_gate is not None
            if repair_contract.get("origin") != "author_self_review":
                errors.append("author-origin repair contract has the wrong origin")
            if (
                repair_contract.get("schema") != "lecture-animation-repair-contract-v2"
                or not validate_hashed_record(repair_contract, "contract_hash")
            ):
                errors.append("author-origin repair contract schema or hash is invalid")
            if repair_contract.get("author_self_review_hash") != expected_author_hash:
                errors.append("author-origin repair contract is bound to another self-review rejection")
            errors.extend(validate_repair_response_data(repair_response, repair_contract, manifest))
            errors.extend(validate_repair_gate_data(repair_gate, repair_response, repair_contract, manifest))
            if repair_context.get("repair_contract_hash") != repair_contract.get("contract_hash"):
                errors.append("author-origin repair self-review contains a stale repair_contract_hash")
            if repair_context.get("repair_response_hash") != object_hash(repair_response):
                errors.append("author-origin repair self-review contains a stale repair_response_hash")
            if repair_context.get("repair_gate_hash") != repair_gate.get("gate_hash"):
                errors.append("author-origin repair self-review contains a stale repair_gate_hash")
            if repair_context.get("repair_execution") != repair_response.get(
                "repair_execution"
            ):
                errors.append(
                    "author-origin repair self-review contains a stale repair_execution record"
                )
    elif isinstance(repair_context, dict) and (
        repair_context.get("previous_review_hash")
        or repair_context.get("previous_author_self_review_hash")
    ):
        errors.append("initial self-review cannot claim an unverified prior rejection")
    return errors
...
def load_repair_bundle_for_self_review(
    ...
    previous_author_path = getattr(args, "previous_author_self_review", None)
    previous_author_self_review = (
        load_json(Path(previous_author_path)) if previous_author_path else None
    )
    if previous_review is not None and previous_author_self_review is not None:
        raise PipelineError("repair self-review cannot bind both prior rejection types")
    if previous_review is None and previous_author_self_review is None:
        if any(paths):
    ...
    if previous_review is not None:
        if contract.get("origin") == "author_self_review":
            raise PipelineError(
                "independent-review repair cannot use an author-origin contract"
            )
        ...
        assert previous_author_self_review is not None
        if contract.get("origin") != "author_self_review":
            raise PipelineError("author-origin repair contract has the wrong origin")
        if contract.get("author_self_review_hash") != object_hash(previous_author_self_review):
            raise PipelineError("repair contract is bound to another author self-review rejection")
    if (
        contract.get("schema") != "lecture-animation-repair-contract-v2"
        or not validate_hashed_record(contract, "contract_hash")
    ):
        raise PipelineError("repair contract schema or hash is invalid")
...
def design_readiness_inputs(args: argparse.Namespace) -> tuple[
    Path, dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any], str
]:
    repo_root = Path(args.repo_root).resolve()
    profile = load_json(resolve_stored_path(args.profile, repo_root))
    plan = load_json(resolve_stored_path(args.plan, repo_root))
    design_gate = load_json(resolve_stored_path(args.design_gate, repo_root))
    authoring_qc = load_json(resolve_stored_path(args.authoring_qc, repo_root))
    animatic = resolve_stored_path(args.animatic, repo_root)
    if not animatic.is_file():
        raise PipelineError("design readiness animatic does not exist")
    animatic_sha = artifact_snapshot(animatic, repo_root)["sha256"]
    return animatic, profile, plan, design_gate, authoring_qc, animatic_sha


def command_prepare_design_readiness(args: argparse.Namespace) -> int:
    animatic, profile, plan, design_gate, authoring_qc, animatic_sha = (
        design_readiness_inputs(args)
    )
    draft = design_readiness_draft_data(
        profile, plan, design_gate, authoring_qc, animatic, animatic_sha
    )
    write_json(Path(args.output), draft)
    print(
        json.dumps(
            {
                "design_readiness_draft": args.output,
                "stage_states": len(draft["stage_state_checks"]),
                "transitions": len(draft["transition_checks"]),
            },
            ensure_ascii=False,
        )
    )
...
def command_seal_design_readiness(args: argparse.Namespace) -> int:
    animatic, profile, plan, design_gate, authoring_qc, animatic_sha = (
        design_readiness_inputs(args)
    )
    readiness = load_json(Path(args.input))
    readiness.pop("readiness_hash", None)
    errors = validate_design_readiness_data(
        readiness,
        profile,
        plan,
        design_gate,
    ...
    if errors:
        raise PipelineError("design readiness failed: " + " | ".join(errors))
    readiness["created_at"] = utc_now()
    readiness["readiness_hash"] = object_hash(readiness)
    output = Path(args.output) if args.output else Path(args.input)
    write_json(output, readiness)
    print(
        json.dumps(
            {
                "design_readiness": str(output),
...
def command_prepare_author_self_review(args: argparse.Namespace) -> int:
    ...
    previous_author_self_review = (
        load_json(Path(args.previous_author_self_review))
    ...
    resolutions = []
    prior_findings_source = previous_review or previous_author_self_review
    if prior_findings_source is not None:
        if previous_review is not None and (
            previous_review.get("schema") != "lecture-animation-review-v2"
            or previous_review.get("verdict") != "revise"
        ):
            raise PipelineError("--previous-review must be an independent revise review")
        if previous_author_self_review is not None and (
            previous_author_self_review.get("schema") != "lecture-animation-author-self-review-v2"
            or previous_author_self_review.get("verdict") != "revise_before_independent_review"
        ):
            raise PipelineError(
                "--previous-author-self-review must be a revise_before_independent_review draft"
            )
        response_map = {
            str(item.get("finding_id")): item
        ...
        for finding in prior_findings_source.get("findings", []):
            if not isinstance(finding, dict):
                continue
    ...
    draft = {
        ...
        "repair_context": {
            ...
            "previous_author_self_review_hash": (
                object_hash(previous_author_self_review)
                if previous_author_self_review
                else None
            ),
            ...
            "repair_execution": (
                repair_response.get("repair_execution")
                if repair_response
                else None
            ),
            "resolutions": resolutions,
    ...
    verification_key = object_hash(
        {
            ...
            "previous_author_self_review_hash": previous_author_self_review_hash,
        }
    )
    attempt = {
        ...
        "gate_errors": errors,
        "gate_error_classes": classify_gate_errors(errors),
...
def command_prepare_review_capsule(args: argparse.Namespace) -> int:
    ...
    repair_execution = self_review.get("repair_context", {}).get(
        "repair_execution"
    )
    if isinstance(repair_execution, dict):
        reviewer_agent_id = str(session.get("reviewer_agent_id", "")).strip()
        if reviewer_agent_id in set(
            map(str, repair_execution.get("editor_agent_ids", []))
        ):
            raise PipelineError(
                "review session is not independent from the repair editor"
            )
        if reviewer_agent_id != str(
            repair_execution.get("planned_verifier_agent_id", "")
        ).strip():
            raise PipelineError(
                "review session reviewer does not match the planned fresh verifier"
            )
...
def command_compile_author_repair_contract(args: argparse.Namespace) -> int:
    repo_root = Path(args.repo_root).resolve()
    repair_plan = load_json(Path(args.repair_plan))
    author_self_review = load_json(Path(args.author_self_review))
    manifest = load_json(Path(args.manifest))
    manifest_errors = verify_manifest_data(manifest, repo_root)
    if manifest_errors:
        raise PipelineError(
            "cannot compile author repair contract from a stale manifest: "
            + " | ".join(manifest_errors)
        )
    exhaustion_path = getattr(args, "review_exhaustion", None)
    if exhaustion_path:
        exhaustion = load_json(Path(exhaustion_path))
        exhaustion_errors = validate_review_exhaustion_data(
            exhaustion,
            repair_plan,
            manifest,
            repo_root=repo_root,
        ...
        if exhaustion_errors:
            raise PipelineError(
                "author repair plan exhaustion failed: "
                + " | ".join(exhaustion_errors)
            )
        repair_plan = dict(repair_plan)
        repair_plan["review_exhaustion"] = exhaustion
    episode = resolve_stored_path(str(manifest.get("episode", "")), repo_root)
    canonical_attempt_log = (
        episode / "review" / "evolution" / "author_self_review_attempts.jsonl"
    ).resolve()
    attempt_log = (
        Path(args.author_attempt_log).resolve()
        if args.author_attempt_log
        else canonical_attempt_log
    )
    if attempt_log != canonical_attempt_log:
        raise PipelineError(
            "--author-attempt-log must resolve to the episode canonical "
            "review/evolution/author_self_review_attempts.jsonl"
        )
    expected_draft_hash = object_hash(author_self_review)
    attempts = [
        row
        for row in event_rows(attempt_log)
        if row.get("manifest_hash") == manifest.get("manifest_hash")
        and row.get("draft_hash") == expected_draft_hash
    ]
    if not attempts:
        raise PipelineError(
            "author repair contract requires the canonical gate-rejected self-review attempt"
        )
    author_attempt = attempts[-1]
    profile = load_json(
        resolve_stored_path(
    ...
    plan = load_json(
        resolve_stored_path(
            str(manifest.get("artifacts", {}).get("plan", {}).get("path", "")),
            repo_root,
        )
    )
    previous_review = (
        load_json(Path(args.previous_review))
    ...
    repair_contract, repair_response, repair_gate = (
        load_repair_bundle_for_self_review(args, previous_review, manifest)
    )
    recomputed_gate_errors = validate_author_self_review_data(
        author_self_review,
        manifest,
        profile,
        plan,
    ...
    contract = author_repair_contract_data(
        repair_plan, author_self_review, author_attempt, manifest
    )
    errors = validate_author_repair_contract_data(
        contract,
        repair_plan,
        author_self_review,
        author_attempt,
        manifest,
        repo_root,
        recomputed_gate_errors=recomputed_gate_errors,
    )
    if errors:
        raise PipelineError("author repair contract failed: " + " | ".join(errors))
    write_json(Path(args.output), contract)
    print(
        json.dumps(
            {
                ...
                "author_self_review_attempt_id": contract["author_self_review_attempt_id"],
                "findings": len(contract["findings"]),
                "lineage_counts": contract["lineage_counts"],
            },
            ensure_ascii=False,
...
def command_prepare_repair_response(args: argparse.Namespace) -> int:
    ...
    if int(contract.get("repair_execution_contract_version", 0) or 0) >= 1:
        execution = repair_execution_data(
            ...
            repair_actor_agent_id=args.repair_actor_agent_id,
            planned_verifier_agent_id=args.planned_verifier_agent_id,
            handoff_count=(
                int(args.handoff_count)
                if args.handoff_count is not None
                else -1
            ),
        )
        execution_errors = validate_repair_execution_data(execution, contract)
        if execution_errors:
            raise PipelineError(
                "repair execution contract failed: "
                + " | ".join(execution_errors)
            )
    response = repair_response_draft_data(
        contract, current_manifest, repair_execution=execution
    )
        ...
        json.dumps(
            {
                ...
                "repair_execution": response["repair_execution"],
            },
            ensure_ascii=False,
            indent=2,
        )
...
def command_verify_review(args: argparse.Namespace) -> int:
    ...
    repair_execution = author_self_review.get("repair_context", {}).get(
        "repair_execution"
    )
    if isinstance(repair_execution, dict):
        editor_ids = set(map(str, repair_execution.get("editor_agent_ids", [])))
        repair_actor_agent_id = str(
            repair_execution.get("repair_actor_agent_id", "")
        ).strip()
        if repair_actor_agent_id != author_agent_id:
            errors.append(
                "repair actor must match the repaired candidate's sealed author_agent_id"
            )
        if reviewer_agent_id in editor_ids:
            errors.append(
                "reviewer_agent_id is recused because it edited the repaired candidate"
            )
        if reviewer_agent_id != str(
            repair_execution.get("planned_verifier_agent_id", "")
        ).strip():
            errors.append(
                "reviewer_agent_id does not match the repair execution's planned fresh verifier"
            )
    if session.get("review_role") == "recusal_acceptance":
        if not isinstance(repair_execution, dict):
            errors.append(
                "recusal_acceptance requires a sealed reviewer-assisted repair execution"
            )
        else:
            if repair_execution.get("mode") != "reviewer_assisted":
                errors.append(
                    "recusal_acceptance requires reviewer_assisted repair mode"
                )
            if str(
                repair_execution.get("discovery_reviewer_agent_id", "")
            ).strip() != str(session.get("main_agent_id", "")).strip():
                errors.append(
                    "recusal_acceptance requires the recused main agent to be the discovery reviewer"
                )
    ...
    if getattr(args, "lint_only", False):
        print(
            json.dumps(
                {
                    "schema": "lecture-animation-review-submission-lint-v2",
                    "valid": not errors,
                    "errors": errors,
                    "gate_error_classes": classify_gate_errors(errors),
                    "findings_count": len(review.get("findings", [])),
                    "finding_classes": classify_findings(review.get("findings", [])),
...
def command_migrate_review_session(args: argparse.Namespace) -> int:
    """Rebind an active review ledger without discarding attempts or pending repairs."""
    repo_root = Path(args.repo_root).resolve()
    source_path = Path(args.input)
    # Migration is the one command that must be able to open a structurally
    # valid ledger bound to an older rules registry. All other commands keep
    # the strict stale-session rejection.
    session = load_review_session(source_path, allow_stale_rules=True)
    reason = str(args.reason or "").strip()
    if len(reason) < 24:
        raise PipelineError("review-session migration requires a concrete --reason")
    spine_path = resolve_stored_path(str(args.episode_spine), repo_root)
    spine = load_json(spine_path)
    if spine.get("schema") != "lecture-animation-episode-visual-spine-v2" or not validate_hashed_record(spine, "spine_hash"):
        raise PipelineError("review-session migration requires a valid current episode spine")
    previous_author_agent_id = str(session.get("author_agent_id", "")).strip()
    previous_owner = str(session.get("owner", "")).strip()
    requested_author_agent_id = getattr(args, "author_agent_id", None)
    requested_owner = getattr(args, "owner", None)
    author_agent_id = str(requested_author_agent_id or previous_author_agent_id).strip()
    owner = str(requested_owner or previous_owner).strip()
    if not author_agent_id or not owner:
        raise PipelineError("review-session migration requires a non-empty owner and author_agent_id")
    author_changed = author_agent_id != previous_author_agent_id or owner != previous_owner
    if author_changed and (not requested_author_agent_id or not requested_owner):
        raise PipelineError("author reassignment requires both --owner and --author-agent-id")
    reviewer_agent_id = str(args.reviewer_agent_id).strip()
    governance_facts, governance_errors = review_session_governance(
        spine,
        reviewer_agent_id=reviewer_agent_id,
        author_agent_id=author_agent_id,
        review_role=str(session.get("review_role", "acceptance")),
    )
    if governance_errors:
        raise PipelineError("review-session migration governance failed: " + " | ".join(governance_errors))
    if reviewer_agent_id == author_agent_id:
        raise PipelineError("migrated reviewer_agent_id must differ from author_agent_id")
    previous_identity = {
        "owner": session.get("owner"),
        "author_agent_id": session.get("author_agent_id"),
        "reviewer": session.get("reviewer"),
        "reviewer_model": session.get("reviewer_model"),
        "reasoning_effort": session.get("reasoning_effort"),
        "reviewer_agent_id": session.get("reviewer_agent_id"),
        "episode_spine_hash": session.get("episode_spine_hash"),
        "session_hash": session.get("session_hash"),
    }
    history = list(session.get("migration_history", []))
    history.append(
        {
            "migrated_at": utc_now(),
            "reason": reason,
            "previous": previous_identity,
            "preserved_attempt_ids": list(session.get("applied_review_attempt_ids", [])),
            "preserved_pending_repairs": sorted(session.get("pending_repairs", {})),
            "author_reassigned": author_changed,
        }
    )
    session.update(
        {
            ...
            "reviewer": args.reviewer,
            "reviewer_model": args.reviewer_model,
            "reviewer_tier": args.reviewer_tier,
            ...
            **governance_facts,
            ...
            "contract_version": REVIEW_SESSION_CONTRACT_VERSION,
            "reviewer_switches": int(session.get("reviewer_switches", 0) or 0) + 1,
            "revision": int(session.get("revision", 0) or 0) + 1,
            "migration_history": history,
        }
    )
    output = Path(args.output) if args.output else source_path
    save_review_session(output, session)
    migrated = load_review_session(output)
    if migrated.get("applied_review_attempt_ids") != session.get("applied_review_attempt_ids"):
        raise PipelineError("review-session migration lost applied review attempts")
    if migrated.get("pending_repairs") != session.get("pending_repairs"):
        raise PipelineError("review-session migration lost pending repair state")
    print(
        json.dumps(
            {
                "review_session": str(output),
                "session_id": migrated.get("session_id"),
                "revision": migrated.get("revision"),
                "preserved_attempts": len(migrated.get("applied_review_attempt_ids", [])),
                "preserved_pending_repairs": len(migrated.get("pending_repairs", {})),
                "session_hash": migrated.get("session_hash"),
            },
            ensure_ascii=False,
        )
    )
...
def command_prepare_diagnostic_review(args: argparse.Namespace) -> int:
    ...
    author_self_review = load_json(Path(args.author_self_review))
    repair_contract, repair_response, repair_gate = load_repair_bundle_for_self_review(
        args, previous_review, current_manifest
    )
    self_review_errors = validate_author_self_review_data(
        author_self_review,
        current_manifest,
...
DEFAULT_EPISODE_ACTIVE_SECONDS = 8.0 * 3600.0
DEFAULT_RETROSPECTIVE_RESERVE_SECONDS = 45.0 * 60.0
DEFAULT_CLOSURE_RESERVE_SECONDS = 3.25 * 3600.0
...
def efficiency_budget_data(args: argparse.Namespace) -> dict[str, Any]:
    return {
        "schema": "lecture-animation-efficiency-budget-v3",
        "episode_active_seconds": round(
            float(args.episode_target_hours) * 3600.0,
            3,
        ),
        "retrospective_reserve_seconds": round(
        ...
        "closure_reserve_seconds": round(
            float(
                getattr(
                    args,
                    "closure_reserve_minutes",
                    DEFAULT_CLOSURE_RESERVE_SECONDS / 60.0,
                )
            )
            * 60.0,
            3,
...
def validate_efficiency_budget_data(budget: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if budget.get("schema") != "lecture-animation-efficiency-budget-v3":
    ...
    phase_tokens = budget.get("phase_token_fractions")
    if not isinstance(phase_active, dict) or set(phase_active) != phase_keys:
        ...
        phase_active = {}
    if not isinstance(phase_tokens, dict) or set(phase_tokens) != phase_keys:
        errors.append(
            "phase_token_fractions must define every non-wait budget bucket exactly once"
        )
        phase_tokens = {}
    if phase_active:
        try:
    ...
    if phase_tokens:
        try:
            token_values = {
                key: float(phase_tokens[key]) for key in phase_keys
            }
        except (TypeError, ValueError):
            token_values = {}
            errors.append("phase token envelopes must be numeric")
        if token_values and (
            any(value <= 0 for value in token_values.values())
            or sum(token_values.values()) > 1.0 + 1e-9
        ):
            errors.append(
                "phase token envelopes must be positive and sum to at most one"
            )
        if token_values:
            early_tokens = sum(
                token_values[key]
            ...
            if (
                early_tokens
                > 1.0 - closure_token_fraction + 1e-9
            ):
                errors.append(
            ...
            if (
                ...
                errors.append(
                    "closure phase token envelopes consume the retrospective reserve"
                )
            if (
                ...
                errors.append(
                    "retrospective token envelope exceeds its reserve"
                )
    for field in (
...
def active_token_reservations(
    ...
    for reservation in ledger.get("reservations", {}).values():
        ...
        for field in total:
            total[field] += int(allocation.get(field, 0) or 0)
    return total
...
def projected_active_seconds(
    ...
    *,
    ...
    projected_rows = [
        ...
        if row.get("phase") != "human_wait"
        and (
            budget_bucket is None
            or phase_budget_bucket(
                str(row.get("phase", "")),
                str(row.get("phase_purpose", "") or ""),
            )
            == budget_bucket
    ...
    new_bucket = phase_budget_bucket(new_phase, new_phase_purpose)
    ...
    return interval_union_seconds(projected_rows)
...
def command_phase_start(args: argparse.Namespace) -> int:
    ...
    phase_envelope = (
        ...
        if budget_bucket is None
            ...
            "active_seconds": float(
                efficiency_contract["budget"]["phase_active_seconds"][
                    budget_bucket
                ]
            ),
            "token_limits": {
                field: int(
                    int(efficiency_contract["budget"][field])
                    * float(
                        efficiency_contract["budget"][
                            "phase_token_fractions"
                        ][budget_bucket]
                    )
                )
                for field in (
    ...
    if expensive_phase:
        ...
        if not readiness_path:
            raise PipelineError(
                "expensive TTS/final render phase requires --design-readiness from the low-cost animatic gate"
            )
    ...
    with locked_paths(
        [state_path, reservation_ledger_path, central_phase_log]
    ):
        ...
        completed_rows = read_jsonl_unlocked(central_phase_log)
        ...
        reserved = active_token_reservations(reservation_ledger)
        observed = fresh_efficiency_status["token_status"]["observed"]
        ...
        if budget_bucket is not None:
            phase_reserved = active_token_reservations(
                reservation_ledger,
                budget_bucket,
            )
            phase_observed = phase_bucket_token_usage(
                completed_rows,
                budget_bucket,
            )
        ...
        if overdue_reservations and args.phase != "human_wait":
            raise PipelineError(
                "phase-start is blocked by overdue active-time "
                "reservations: "
        ...
        if budget_bucket is not None:
            ...
            if (
                ...
                raise PipelineError(
                    "phase-start active-time reservation exceeds the "
                    f"{budget_bucket} phase envelope"
                )
        explicit_phase_instance_id = getattr(args, "phase_instance_id", None)
        shared_work_key = str(getattr(args, "shared_work_key", "") or "").strip()
        if explicit_phase_instance_id and shared_work_key:
            raise PipelineError("use either --phase-instance-id or --shared-work-key, not both")
        if shared_work_key:
            shared_identity = "|".join(
                [
                    str(args.run_id),
                    str(args.phase),
                    phase_purpose,
            ...
            phase_instance_id = (
                "phase-instance:shared:"
                + hashlib.sha1(shared_identity.encode()).hexdigest()[:16]
            )
        else:
...
def command_phase_end(args: argparse.Namespace) -> int:
    ...
    with locked_paths(lock_targets):
        ...
        **repair_completion,
        ...
        actual_against_allocation = token_budget_observation(
            usage,
            {
                **allocation,
                "warning_fraction": 0.75,
        ...
        if central_phase_log.resolve() != phase_log.resolve():
        ...
        central_rows_at_end = read_jsonl_unlocked(central_phase_log)
        ...
        if budget_bucket and isinstance(phase_envelope, dict):
            bucket_tokens = phase_bucket_token_usage(
                central_rows_at_end,
                budget_bucket,
            )
    ...
    return 2 if (
        allocation_exceeded
        or active_allocation_exceeded
        or phase_envelope_exceeded
    ) else 0
...
def command_batch_status(args: argparse.Namespace) -> int:
    ...
    hard_alerts = {
        ...
        "SHARED_PHASE_IDS_INCONSISTENT",
        "PHASE_TELEMETRY_INCOMPLETE",
        "SCENE_PHASE_PAIRS_INCOMPLETE",
        "TOKEN_TELEMETRY_INCOMPLETE",
        "HUMAN_OUTCOME_LOG_STALE",
        "HUMAN_OUTCOME_SCENES_MISSING",
        "SELF_REVIEW_SEMANTIC_ESCAPE",
        "REVIEW_ARTIFACT_EXPLOSION",
        "EFFICIENCY_BUDGET_INVALID",
        "EPISODE_ACTIVE_BUDGET_EXCEEDED",
        "EPISODE_TOKEN_BUDGET_EXCEEDED",
    ...
    if {
        ...
        return 2
    if bool(getattr(args, "require_clean", False)) and hard_alerts & set(alerts):
        return 2
    return 0
...
def retrospective_evidence_data(repo_root: Path, episode: Path) -> dict[str, Any]:
    ...
    elif approved_masters and passing_portability and final_videos:
...
def command_close_episode_efficiency(args: argparse.Namespace) -> int:
    ...
    receipt = {
        ...
        "status": "completed" if evaluation["compliant"] else "failed",
    }
    output = Path(args.output)
...
def latest_human_passes(event_log: Path) -> dict[str, dict[str, Any]]:
    ...
    for row in event_rows(event_log):
        if row.get("human_verdict") != "pass" or not row.get("scene_slug"):
            continue
        scene_slug = str(row["scene_slug"])
        previous = latest.get(scene_slug)
        if previous is None or str(row.get("created_at", "")) >= str(previous.get("created_at", "")):
            latest[scene_slug] = row
    return latest
...
def validate_supervisor_episode_completion(supervisor: dict[str, Any]) -> dict[str, Any]:
    ...
    if invalid_assignments or invalid_tasks or invalid_replacements:
        raise PipelineError(
            "closed supervisor session has invalid assignment/task/replacement states"
        )
    ...
    if unfinished_agents or unfinished_tasks or pending_replacements:
...
def command_finalize_episode(args: argparse.Namespace) -> int:
    ...
    batch_paths = [resolve_stored_path(value, repo_root) for value in (args.batch or [])]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    state_parser.add_argument("--previous-author-self-review")
    state_parser.add_argument("--repair-contract")
    state_parser.add_argument("--repair-response")
    state_parser.add_argument("--repair-gate")
    ...
    efficiency_parser.add_argument(
        ...
        type=float,
        default=DEFAULT_EPISODE_ACTIVE_SECONDS / 3600.0,
    )
    efficiency_parser.add_argument(
    ...
    efficiency_parser.add_argument(
        ...
        type=float,
        default=DEFAULT_CLOSURE_RESERVE_SECONDS / 60.0,
    )
    efficiency_parser.add_argument(
    ...
    phase_start_parser.add_argument(
        "--phase-purpose",
        help="classify render/TTS work so retry cost and readiness gates are enforceable",
    )
    phase_start_parser.add_argument(
        "--design-readiness",
        help="sealed low-cost animatic readiness receipt required by TTS and final renders",
    )
    phase_start_parser.add_argument(
        "--episode-readiness",
    ...
    phase_start_parser.add_argument(
        ...
        required=True,
        help=(
            "maximum active wall seconds reserved for this phase; "
            "parallel reservations are projected as overlapping intervals"
        ),
    )
    phase_start_parser.add_argument(
        ...
        type=int,
        required=True,
        help="maximum input-plus-output tokens reserved before this phase starts",
    )
    phase_start_parser.add_argument(
    ...
    phase_start_parser.add_argument(
        "--shared-work-key",
        help="derive one deterministic phase_instance_id for batch work reused by several scenes",
    )
    phase_start_parser.add_argument(
        "--prompt-bytes",
        type=int,
        default=0,
        help="bounded assignment prompt bytes; hard-capped per task capsule",
    )
    phase_start_parser.add_argument(
        "--artifact-input-bytes",
        type=int,
        default=0,
        help="bytes of text/structured artifacts intentionally loaded into context",
    )
    phase_start_parser.add_argument(
        "--files-read",
        type=int,
        default=0,
        help="number of bounded source/policy artifacts intentionally read for this task",
    )
