...
PASS_REVIEW_ROLES = {"acceptance", "recusal_acceptance"}
...
def review_session_governance(
    ...
    *,
    ...
    if review_role not in {
        "acceptance",
        "recusal_acceptance",
        "diagnostic_support",
    }:
        errors.append(
            "review_role must be acceptance, recusal_acceptance, or diagnostic_support"
        )
    ...
    if production_mode == "parallel_batches":
        ...
        elif review_role == "recusal_acceptance":
            if author_agent_id != main_agent_id:
                errors.append(
                    "recusal_acceptance requires the episode main agent to be the repaired candidate author"
                )
            if reviewer_agent_id == main_agent_id:
                errors.append(
                    "recusal_acceptance must be performed by a fresh verifier, not the recused main agent"
                )
    elif review_role == "recusal_acceptance":
        errors.append(
            "recusal_acceptance is reserved for parallel episodes where the main agent edited a repaired candidate"
        )
    return (
        {
...
def validate_session_governance(
    ...
    if verdict == "pass_for_user_review_pending" and session.get("review_role") not in PASS_REVIEW_ROLES:
        errors.append(
            "diagnostic_support reviewers cannot grant pass_for_user_review_pending; "
            "only acceptance or recusal_acceptance may do so"
        )
    return errors
