...
class StorageConcurrencyTests(unittest.TestCase):
    ...
    def test_gate_rejected_submission_is_logged_but_not_counted_as_review_round(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            session_path = root / "session.json"
            attempt_log = root / "attempts.jsonl"
            session = {
                "schema": "lecture-animation-review-session-v2",
                "session_id": "session-rejected-submission",
                "scenes": [],
                "full_reviews": 0,
                "calibration_scene_interval": 1,
                "calibration_due": False,
                "revision": 0,
                "applied_review_attempt_ids": [],
                "pending_repairs": {},
            ...
            write_json(session_path, session)
            rejected = {
                "attempt_id": "rejected-1",
                "verification_key": "rejected-key",
                "gate_accepted": False,
                "verdict": "revise",
                "gate_error_classes": {"schema_or_contract": 3},
            }
            _, appended, current = commit_review_attempt(
                session_path=session_path,
                attempt_log=attempt_log,
                expected_session_hash=session["session_hash"],
                attempt=rejected,
                scene_slug="g001",
                manifest_hash="manifest-invalid",
                calibration_performed=False,
                reviewer_anomalous=False,
            )
            self.assertTrue(appended)
            self.assertEqual(current["full_reviews"], 0)
            self.assertEqual(current["scenes"], [])
            self.assertFalse(current["calibration_due"])
            self.assertEqual(current["gate_rejected_attempts"], 1)
            self.assertEqual(current["review_submission_attempts"], 1)
