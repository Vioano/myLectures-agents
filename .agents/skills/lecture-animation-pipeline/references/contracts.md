...
- v6 `visual_finish_contract` inside the representation budget: scene-specific
  ...
  composition obligations already represented by the animatic; only permitted
...
- v7 `screen_text_contract.semantic_items`, covering every literal payload in
  ...
  role, and a unique visual job. Long explanatory prose, narration duplicates,
  creator/process commentary, and unregistered dynamic text are blockers;
...
New autopilot profiles use contract version 7. Supported display modes are
...
- missing `visual_finish_checks`, an animatic that defers composition or
  hierarchy to formal rendering, unreadable thumbnail structure, unowned
...
## Design Readiness Receipt

Before scene-local TTS or a candidate/repair render, prepare and seal `lecture-animation-design-readiness-v2` from the current profile, scene plan, design gate, passing low-cost authoring QC, and animatic file. The receipt requires:

- full animatic playback plus a muted novice teach-back and driver prediction;
- one concrete check for every stage state and every transition;
- no unresolved design issue;
- simultaneous formula-memory evidence for formula-dense scenes instead of a single formula slot;
- `design_frozen: true` and verdict `ready_for_audio_lock`.

`phase-start --phase render --phase-purpose smoke|animatic` may run before this receipt. `candidate`, `repair_rerender`, and `technical_retry` renders, plus every TTS phase, require `--design-readiness`. This turns “animatic before audio” from prose advice into a hard cost gate.

The receipt embeds `scene_complexity_gate`. A duration up to 75 seconds is within target; 75-90 seconds is a recorded warning; more than 90 seconds is blocked. The only exception is a plan-owned `scene_split_exception` with a concrete reason, at least two `internal_sections` bound to `stage_state_ids`, one or more `clearance_checkpoints`, and a novice-continuity reason. The exception is hash-bound to the plan and cannot be added as an untracked production note.
...
The default roster cap is three subagents, excluding the main supervisor. A
larger roster requires a concrete capacity override. The main agent normally
uses those three slots for production owners and remains the independent
acceptance reviewer.
...
`assign-task` is the normal rolling transition. It accepts only an `idle` or
`completed` roster identity, preserves its role, appends a task-history row, and
increments `reuse_count` without changing `identity_history`. The orchestration
...
After the initial roster, creating a new identity is a two-step exceptional
transition: `authorize-replacement`, then `register-replacement`. Authorization
first checks that no compatible idle roster member exists. Accepted reason keys
are `agent_unavailable`, `task_tree_changed`, `model_change_required`, and
...
Availability snapshots expire after fifteen minutes. Replacement authorization
rejects a missing probe, a `restored` outcome, or any id still marked reusable.
Only the three explicit failure outcomes can support replacement. A thread or
checkpoint supplies context, but the direct canonical child id remains the
first recovery target. Model-change authorization must name a model different
from the frozen current model. The default replacement budget is one;
...
session may be restarted only with a recorded reason. Version-1 sessions remain
readable for historical status, but roster mutations require version 2.
...
`begin-episode-efficiency` must run in the canonical production checkout before planning. It writes `lecture-animation-episode-efficiency-contract-v3`, binding the repo-relative episode, canonical checkout root, shared episode phase ledger, eight-hour budget, token ceilings, a 195-minute closure reserve, a 45-minute retrospective reserve, 32-percent closure token reserves, 7-percent retrospective token reserves, phase-specific active-time/token envelopes, and quality targets. An active contract is idempotent and cannot be recreated to reset prior consumption; an orphan nonempty central ledger also blocks a fresh contract.

`phase-start` requires the episode, active efficiency contract, one active-wall-time allocation, and four explicit task-capsule token allocations: raw input plus output, uncached input, output, and reasoning. Under one deterministic multi-file lock it reads completed usage and the canonical reservation ledger. Token reservations are summed; active-time reservations are projected as timestamped intervals so concurrent work overlaps rather than being double-counted. Early work is checked against the episode budget minus the closure reserve, closure work against the budget minus the retrospective reserve, and retrospective against the complete outer ceiling. The same atomic transition also checks the applicable phase envelope: planning 45 minutes/5 percent, design 45/10, authoring 75/25, render 75/20, TTS 25/5, ASR 20/3, review 60/12, repair 30/8, finalization 60/5, retrospective 45/7. Repair rerenders, technical retries, pronunciation retries, post-readiness script changes, and reuse verification charge the repair envelope rather than silently consuming initial render/TTS/ASR capacity. A successful start creates both a hash-stamped active timer and an active reservation with actor role, model, reasoning effort, prompt bytes, artifact bytes, files read, budget bucket, phase envelope, and a reusable `phase_instance_id`. It snapshots cumulative token usage from `--usage-file`, `LECTURE_TOKEN_USAGE_FILE`, or the current Codex rollout discovered through `CODEX_THREAD_ID`. `phase-end` records the nonnegative delta in both the requested local log and the canonical shared ledger, releases the reservation, and fails when actual wall time or token use exceeded any reserved amount or the sealed phase envelope; explicit token arguments are a fallback for workers that expose usage only at completion. Shared concurrent work must pass the same `--shared-work-key`; the CLI derives the same deterministic instance ID across scene wrappers so the work is counted once. `human_wait` must reserve zero active time and zero tokens. Use one of: `planning`, `design`, `authoring`, `render`, `review`, `repair`, `tts`, `asr`, `finalization`, `retrospective`, `human_wait`.
...
`record-outcome` derives `outcome_key` from the complete outcome payload and appends it exactly once, so retrying a command does not inflate human-review counts. `finalize-episode` is the only normal media/production close transition. It requires one durable human-pass outcome per scene, terminal issue statuses, scene-local exact artifacts, required final media/timing artifacts, and complete batch coverage in parallel mode. Parallel completion additionally requires the v2 supervisor session to have passed `finish`, with no active/blocked assignment, pending/blocked task, or unused replacement authorization. The completion receipt records historical identity and replacement counts. Finalization atomically marks scenes `assembled`, closes supplied batch contracts, seals the final assembly, and writes `lecture-animation-episode-completion-v2`. A missing outcome, stale open issue, provisional scene, missing file, uncovered parallel scene, or live supervisor roster blocks completion.

After the retrospective phase, `close-episode-efficiency` is the final process-compliance transition. It validates the production completion receipt, canonical phase ledger, required global phases through retrospective, per-scene phase pairs, token observability, active time, all four token ceilings, valid task-capsule resource evidence on every completed non-wait event, zero known-regression recurrence, zero automatic false pass, and human-issue scene rate below the sealed quality threshold. Failure writes a durable failed audit while leaving the efficiency contract active; success atomically marks the contract completed and writes `lecture-animation-episode-efficiency-close-v1`. Tool installation or a same-episode snapshot is not evidence of success without this receipt from a matched future episode.
