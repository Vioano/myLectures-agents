...
2. Rank patterns by human misses, recurrence, and time cost.
3. Promote only the few highest-value candidates.
4. Merge rules with the same evidence test and applicability.
5. Retire rules that have not triggered, are consistently marked not applicable, or add review time without catching failures.
6. Re-run profile compilation tests so a typical scene still receives a small rule subset.
