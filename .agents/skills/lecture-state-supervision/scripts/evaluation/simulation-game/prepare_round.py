...
SKILL_ROOT = THIS_ROOT.parents[2]
SYSTEM_ROOT = SKILL_ROOT / "scripts" / "runtime"
EVALUATION_ROOT = SKILL_ROOT / "scripts" / "evaluation"
INSTALLED_PROJECT_ROOT = (
    SKILL_ROOT.parents[2]
    if SKILL_ROOT.parent.name == "skills" and SKILL_ROOT.parent.parent.name == ".agents"
    else Path.cwd()
)
PROJECT_ROOT = Path(os.environ.get("LECTURE_REPO_ROOT", INSTALLED_PROJECT_ROOT)).resolve()
DEFAULT_RESULTS = Path(
    os.environ.get(
        "LECTURE_STATE_SIMULATION_RESULTS",
        str(PROJECT_ROOT / "review" / "state-supervision" / "simulation-game" / "results"),
    )
)
...
def copy_public_inputs(repo: Path) -> None:
    copy_map = {
        ...
        SKILL_ROOT / "references" / "operator-guide.md": repo / "public" / "OPERATOR_GUIDE.md",
        THIS_ROOT / "fixtures" / "tts-pronunciation-registry.md": repo
        / "references/tts-pronunciation-registry.md",
        THIS_ROOT / "fixtures" / "math-object-driven-animation.md": repo
        / "references/math-object-driven-animation.md",
        THIS_ROOT / "fixtures" / "scene-production-and-review.md": repo
